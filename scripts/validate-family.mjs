...
assert(readmeText.startsWith("![Avenor icon](assets/avenor.png)\n\n# Avenor\n"),
  "README hero must begin with the Avenor icon and product name");
assert(readmeText.includes("Maximum Clarity. Minimum Noise."), "README is missing the brand principle");
assert(readmeText.includes("## Avenor Signature"), "README is missing the flagship section");
...
for (const variant of variants) {
  ...
  assert(readmeText.includes(`assets/screenshots/avenor-${slug}.png`),
    `README screenshot structure is missing ${variant.name}`);
}
